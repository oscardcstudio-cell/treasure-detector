...
| Forêt publique (domaniale/communale, ONF) | WMTS `FORETS.PUBLIQUES` · WFS `BDTOPO_V3:foret_publique` (`data.geopf.fr/wfs/ows`) | Distinguer accès toléré (forêt gérée) de privé — voir `src/foncier` | [FAIT — permalink Géoportail + GetFeature réel vérifiés 2026-08-10] |
| Voies et chemins | WFS `BDTOPO_V3:troncon_de_route` (`data.geopf.fr/wfs/ows`), champ `prive` (bool) | Distinguer voie publique de voie privée — voir `src/foncier` | [FAIT — GetFeature réel vérifié 2026-08-10] |
| PCI Vecteur Etalab (bâtiments + parcelles) | `cadastre.data.gouv.fr/data/etalab-cadastre/latest/geojson/communes/{dept}/{insee}/cadastre-{insee}-{batiments,parcelles}.json.gz` | Fichiers séparés — bâti = exclusion foncier directe | [FAIT — pattern testé sur commune 32009, GetFeature réel vérifié 2026-08-10] |
...
| **Lit de rivière/ruisseau à sec (étiage)** | All-Metal | 6-7 → **4-5** sur les dépôts argileux/vase exposés | stock (sniper si ferraille dense) | **transects perpendiculaires à l'axe** (chercher le resserrement = gué) + le long des berges découvertes | tout signal répétable ; trier le fer agricole (clôtures, matériel tombé à l'eau) | monnaies perdues au passage, ferrures de gué, objets charriés et déposés en aval d'un obstacle (racine, rocher) |
...
| 11 | Critère `proximity_source` (rivières/gués, poids 12) **inactif** — `data/derived/hydro_streams.geojson` n'existe pas | Zones proches d'un cours d'eau non valorisées par le score aujourd'hui (6/8 critères actifs) | Script prêt : `tools/prep/hydro.py` (WFS `BDTOPO_V3:troncon_hydrographique`) — **[MACHINE LOCALE]**, proxy egress bloque `data.geopf.fr` en conteneur (reconfirmé 2026-08-11). Moteur corrigé côté code (distance point-segment sur LineString, `src/scoring/engine.ts`, testé) — reste à faire une fois la donnée réelle en main : le chargement multi-source dans `scoreZone`/`ScoringLayer.tsx` (aujourd'hui un seul GeoJSON, `toponymes.geojson`, est chargé pour tous les critères) | Oscar (exécution WFS) + T3.1/T3.3 (chargement multi-source) |
